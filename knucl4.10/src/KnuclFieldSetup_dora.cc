...
KnuclFieldSetup_dora::KnuclFieldSetup_dora(const char* field_map, double xOffset, double yOffset, double zOffset, double norm) 
  : fChordFinder(0), fEquation(0), fMagneticField(0), fStepper(0)
{    
  fMinStep     = 0.25*mm ;
  fMagneticField = new dora_field(field_map, xOffset, yOffset, zOffset, norm);
...
KnuclFieldSetup_dora::~KnuclFieldSetup_dora()
{
  if(fMagneticField) delete fMagneticField;
  if(fChordFinder)   delete fChordFinder;
  if(fStepper)       delete fStepper;
  if(fEquation)      delete fEquation;
  G4cout<<"KnuclFieldSetup_dora::~KnuclFieldSetup_dora()"<<G4endl;
  //  getchar();
}

dora_field::dora_field( const char* filename, double xOffset, double yOffset, double zOffset, double norm)
  :norm_factor(1.0), fXoffset(xOffset),fYoffset(yOffset),fZoffset(zOffset),invertX(false),invertY(false),invertZ(false)
{    
  ...
  {
    G4cout << "sample value from dora field map: z=-50cm " <<G4endl;
    const double test_point2[4]={0*cm, 0*cm, -50*cm, 0};
    double test_field2[3]={-1, -1, -1};
    GetFieldValue(test_point2, test_field2);
    G4cout << test_field2[0]/gauss << " " << test_field2[1]/gauss << " " << test_field2[2]/gauss << " gauss "
	   << "\n-----------------------------------------------------------" << endl;
  }

  {
    G4cout << "sample value from dora field map: z=40cm " <<G4endl;
    const double test_point2[4]={0*cm, 0*cm, 40*cm, 0};
    double test_field2[3]={-1, -1, -1};
    GetFieldValue(test_point2, test_field2);
    G4cout << test_field2[0]/gauss << " " << test_field2[1]/gauss << " " << test_field2[2]/gauss << " gauss "
	   << "\n-----------------------------------------------------------" << endl;
...
void dora_field::GetFieldValue(const double point[4],
				      double *Bfield ) const
{
