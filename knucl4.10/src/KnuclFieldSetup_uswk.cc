...
KnuclFieldSetup_uswk::KnuclFieldSetup_uswk(const char* field_map, double xOffset, double yOffset, double zOffset) 
  : fChordFinder(0), fEquation(0), fMagneticField(0), fStepper(0)
{    
  fMinStep     = 0.25*mm ;
  fMagneticField = new uswk_field(field_map, xOffset, yOffset, zOffset);
...
KnuclFieldSetup_uswk::~KnuclFieldSetup_uswk()
{
  if(fMagneticField) delete fMagneticField;
  if(fChordFinder)   delete fChordFinder;
  if(fStepper)       delete fStepper;
  if(fEquation)      delete fEquation;
  G4cout<<"KnuclFieldSetup_uswk::~KnuclFieldSetup_uswk()"<<G4endl;
  //  getchar();
}

uswk_field::uswk_field( const char* filename, double xOffset, double yOffset, double zOffset) 
...
void uswk_field::GetFieldValue(const double point[4],
				      double *Bfield ) const
{
