...
int main( int argc, char** argv )
{
  ...
  for (Int_t i=0; i<nevent; i++) {
    ...
    const int npart=7;
    //int PDG[npart] = {321, 211, -211, 2112, 3122,2212,-211};
    int PDG[npart] = {321, 211, -211, 2112, 3122,2112,111};
...
//    bool pippimn_detect1 =  (nCDHhit[1] && nCDHhit[2] && nCDHhit[3]) ? true : false;
    ...
    if(pippimn_detect1) std::cout << "yes 1" << std::endl;
                              //prompt pi+, decay pi-,   prompt n
    bool pippimn_detect2 =  (nCDHhit[1] && nCDHhit[3] && nCDHhit[5]) ? true : false;
    ...
    for(int ip=0;ip<npart;ip++){
      if(ip) TL_mcData[ip].SetVectM(mcData->track(ID[ip])->momentum()*0.001,pdg->GetParticle(PDG[ip])->Mass());
      else  TL_mcData[ip].SetVectM(mcData->track(ID[ip])->momentum()*-0.001,pdg->GetParticle(PDG[ip])->Mass());
    ...
    pimom->Fill(TL_mcData[1].P());
    pimom->Fill(-1.*TL_mcData[2].P());
    if(cos_n<0.7)IMnpim_IMnpip1->Fill(TL_mcData_npim.M(),TL_mcData_npip.M());
