...
void nc_singleeff(const char* filename="")
{
  ...
  //for (Int_t i=0; i<500; i++) {
    if(i%5000==1) std::cout << i  << std::endl;
    tree->GetEvent(i);
    ...
    for (Int_t j=0; j<detectorData->detectorHitSize(); j++) {
      ...
      if( cid == CID_CDH ){
        ...
        if(dE>threshold){
          ...
          Tools::H1(Form("ncan_pdg"),pdgid,16000,-8000,8000);
          Tools::H1(Form("ncan_parentpdg"),parentpdg,16000,-8000,8000);
          double ParentVtxR = FillAncestryVertexR(mcData,detectorData->detectorHit(j),  dE);
          hdet->Fill(momentum/1000.0);
          if(fabs(ParentVtxR<1.0))hdet_pri->Fill(momentum/1000.0);
        }
      }
    }
...
//  c2->Print("tmp.pdf)");
  ...
  TFile *fout = new TFile("ncout.root","RECREATE");
  fout->cd();
